...
export const getAuthClientForUser = async (userId) => {
...
// Calendar Sync
export const syncFuncionToCalendar = async (userId, funcionId) => {
    ...
    const syncToSpecificCalendar = async (calendarId) => {
...
const getOrCreateRootFolder = async (drive) => {
...
export const createObraDriveFolder = async (userId, obraId) => {
    const auth = await getAuthClientForUser(userId);
...
export const listFilesFromDriveFolder = async (userId, folderId) => {
    const auth = await getAuthClientForUser(userId);
    const drive = google.drive({ version: 'v3', auth });
    try {
...
export const uploadFileToDrive = async (userId, folderId, file) => {
    const auth = await getAuthClientForUser(userId);
    const drive = google.drive({ version: 'v3', auth });
    try {
        const res = await drive.files.create({
...
export const listCalendarEvents = async (userId) => {
    ...
    try {
        ...
        try {
            ...
            const hthCalendar = calendarList.data.items?.find(c => c.summary?.toLowerCase().includes('giras hth') ||
